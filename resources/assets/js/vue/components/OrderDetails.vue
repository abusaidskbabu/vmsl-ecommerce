                                        ...
                                        <ul v-if="single_order">
                                            ...
                                            <li v-if="single_order.parent_order_id">
                                                <p class="text-danger">This order has been autometically generated by system.
                                                    Parent Order ID is NJ{{single_order.created_at|prefix_year}}{{single_order.parent_order_id}}. If you want to cancel/renew/update this auto renewal order system  <router-link :to="{name: 'orderDetails', params: {id: single_order.parent_order_id } }">click here</router-link> . </p>
                                            </li>
                                            ...
                                            <li> <b>{{ $t('Order ID') }}: </b> <span>  NJ{{single_order.created_at|prefix_year}}{{ single_order.id }} </span>  </li>
...
<section id="print_invoice">
            ...
            <tr>
                <td style="width: 650px;">
                    <div class="row" style="display: flex; -ms-flex-wrap: wrap;flex-wrap: wrap;padding: 0!important;">
                        ...
                        <div class="col-md-6" style="flex: 0 0 45%;max-width: 45%;position: relative;text-align: right!important;padding: 0!important;">  <span style="text-transform: uppercase;font-size: 14px;font-weight: 600;">  Order ID: NJ{{single_order.created_at|prefix_year}}{{ single_order.id }} </span> </div>
                    </div>
                </td>
            </tr>
        </tbody>
    ...
    <table style="width: 650px;text-align:left; margin:50px auto">
        <tbody style="text-align: center;">
            <p style="font-size:12px;text-align: center;">Thank you for being with us. Stay connected with <b>nurtaj.com.bd</b> </p>
...
export default {
	...
	methods:{
        ...
        load_single_order(){
            ...
            axios.get(this.$baseUrl + "/api/v1/get-single-order/"+order_id, axiosConfig).then((response) => {
                axios.get("/api/barcode/"+'NJ'+moment(String(response.data.order.created_at)).format('YY')+response.data.order.id).then((response) => {
	...
	mounted(){
        ...
        this.load_single_order();
        document.title = "Vmsl | My Order Details";  
	}
}
</script>