...
class ApiController extends Controller
{
		...
		foreach ($products as $item) {
			...
			if ($item->meta) {
				foreach ($item->meta as $meta) {
					if ($meta->meta_key == 'custom_options' || $meta->meta_key == 'product_shipping_option' || $meta->meta_key == 'product_miscellaneous_information') {
						$meta->meta_value = unserialize($meta->meta_value);
						if ($meta->meta_key == 'custom_options') {
							$quantities = [];
							$specificStock = [];
   ...
   //  			}
	...
	public function suggetionProduct($content)
	{
	...
	public function Order(Request $request){
		...
		if ($user) {
			...
			//Customer
			$message = [
				'order_id' =>  'NJ' . date('y', strtotime(Carbon::now())) . $order_id,
				'type' => 'order',
			...
			foreach ($sellers_id_for_order as $key => $val) {
				...
				\Helper::sendSms($seller->phone, 'একটি নতুন অর্ডার সফলভাবে স্থাপন করা হয়েছে! অর্ডার আইডি হল  #' . 'NJ' . date('y', strtotime(Carbon::now())) . $order_id);
			}

			// Customer
			\Helper::sendSms($user->phone, 'অর্ডার সফলভাবে স্থাপন করা হয়েছে! আপনার অর্ডারের জন্য আপনাকে ধন্যবাদ. আপনার অর্ডার আইডি #' . 'NJ' . date('y', strtotime(Carbon::now())) . $order_id);
	...
	public function getSearchProduct(Request $request){
		...
		if ($categories) {
			...
			$subquery = "FIND_IN_SET(0,category_id)";
			if ($categories_ids) {
				foreach ($categories_ids as $catId) {
		...
		if ($request->productspage == 1) {
			if ($request->brands == null && $request->categories == null && $content == "undefined" && $request->orederbyselect == "undefined" && $request->minprice == "undefined" && $request->maxprice == "undefined") {
				$products = product::where('is_active', 1)->where('is_deleted', 0)->where('product_qc', 1)->orderBy('shuffle_number', 'DESC')->paginate(12);
			}
		}
		foreach ($products as $product) {
			$product->price_after_offer = number_format((int)\Helper::price_after_offer($product->id), 0);
