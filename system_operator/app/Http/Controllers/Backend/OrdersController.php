...
class OrdersController extends Controller
{
    ...
    public function getOrderList($start_date, $end_date)
    {
        ...
        return Datatables::of($data)->addIndexColumn()
            ...
            //     return 'NJ' . date('y', strtotime($row->created_at)) . $row->id;
            // })
            ...
            ->editColumn('parent_order_id', function ($row) {
                if ($row->parent_order_id) {
                    return 'NJ' . date('y', strtotime($row->created_at)) . $row->parent_order_id;
                } else {
                    return 'ONE TIME';
                }
            })
    ...
    public function update(Request $request)
    {
        ...
        if (Auth::user()->getRoleNames() != '["seller"]' && $status == 6) {
            ...
            if ($user) {
                $mobile = $user->phone;
                $msg = 'আপনার আর্ডার আইডি #'. 'NJ' . date('y', strtotime($order_details->created_at)) . ' এর একটি পন্য সফলভাবে বিতরণ করা হয়েছে । অনুগ্রহ করে পন্যটির বিষয়ে আপনার মতামত দিন !';
                if ($mobile) {
                    \Helper::sendSms($mobile, $msg);
                }

                $message = [
                    'order_id' => 'NJ'. date('y', strtotime($order_details->created_at)) . $order_details->order_id,
                    'type' => 'order',
                    'message' => $msg,
                ];
            ...
            Helper::sendSms($user->phone, 'আপনার অর্ডার সফলভাবে আপডেট করা হয়েছে! অর্ডার আইডি '.'NJ' . date('y', strtotime($order_details->created_at)) . $order_details->order_id);
    ...
    public function getSellerAccountHistory($id)
    {
        ...
        return DataTables::of($data)->addIndexColumn()
            ->editColumn('order_id', function ($row) {
                return 'NJ' . date('y', strtotime($row->created_at)) . $row->order_id;
            })
