...
class PosController extends Controller
{
    ...
    public function order(Request $request)
    {
        ...
        if ($user) {
            ...
            foreach ($sellers_id_for_order as $key => $val) {
                ...
                \Helper::sendSms($seller->phone, 'একটি নতুন অর্ডার সফলভাবে স্থাপন করা হয়েছে! অর্ডার আইডি হল  #' .'NJ' . date('y', strtotime($order->created_at)) . $order_id);
            }

            // Customer
            \Helper::sendSms($user->phone, 'অর্ডার সফলভাবে স্থাপন করা হয়েছে! আপনার অর্ডারের জন্য আপনাকে ধন্যবাদ. আপনার অর্ডার আইডি #'. 'NJ' . date('y', strtotime($order->created_at)) . $order_id);
    ...
    public function order_function(Request $request){
        ...
        if ($user) {
            ...
            //Customer
            $message = [
                'order_id' =>  'NJ' . date('y', strtotime($order->created_at)) . $order_id,
                'type' => 'order',
                'message' => 'Order placed successfully!',
            ];
