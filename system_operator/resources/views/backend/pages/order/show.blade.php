    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-body formated_para">
                ...
                <hr>
                <p><strong>Order ID: </strong> NJ{{ date('y', strtotime($order->created_at)) }}{{ $order->id }}</p>
                @if ($order->parent_order_id)
                    <p><strong>Parent Order ID: </strong> <a
                            href="/admin/orders/view/{{ $order->parent_order_id }}">NJ{{ date('y', strtotime($order->created_at)) }}{{ $order->parent_order_id }}</a>
                    </p>
                @endif
